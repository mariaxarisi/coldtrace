...
PERF?=		    perf
PERF_EVENT?=
PERF_RECORD=	$(PERF) record -g --call-graph $(CALL_GRAPH) -F $(FREQ) $(PERF_EVENT) \
		        -o $(WORKDIR)/$*.perf.data --

TESTER_PROFILE=	$(PERF_RECORD) sh -c

TESTER_RUN!=	if [ -n "$(NO_HYPERFINE)" ]; then \
...
TESTER=		$(if $(PROFILE),$(TESTER_PROFILE),$(TESTER_RUN))
