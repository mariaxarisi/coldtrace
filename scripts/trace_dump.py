...
class TraceReader:
    ...
    def _read_file(self, path: Path) -> Iterator[TraceEntry]:
        ...
        git_hash, padding, major, minor, patch = VERSION_HEADER.unpack_from(buffer)
        # Headers are informational here so traces from another build remain
        # inspectable, matching the original dumper behavior.
        if self.debug:
            self._debug(
        ...
        while offset < file_size:
            ...
            if header_end > file_size:
                if not any(buffer[offset:file_size]):
                    if self.debug:
                        self._debug(
                            f"{path}:{offset}: reached short zero-filled tail"
                        )
                    break
                raise self._format_error(
                    path,
            ...
            # Memory accesses dominate real traces, so keep their successful
            # decode path free of generic helper calls.
            ...
            if entry is None:
                if self.debug:
                    self._debug(
                        f"{path}:{entry_offset}: reached zero-filled tail"
                    )
                break
            if self.debug:
                self._debug(f"{path}:{entry_offset}: decoded {entry}")
            yield entry
