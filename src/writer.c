...
ensure_buffer_(struct writer_impl *impl)
{
    ...
    impl->comp_buf = malloc(LZ4_compressBound((int)impl->size));
...
coldtrace_writer_reserve(struct coldtrace_writer *ct, size_t size)
{
    ...
    // An entry that cannot fit in an empty fragment can never be written
    if (unlikely(size + sizeof(struct version_header) > trace_size)) {
        log_warn("entry size %zu exceeds fragment capacity", size);
        return NULL;
    }
